...
Double_t v2BackgroundPol3Y(Double_t *x, Double_t*par){
    return par[0] + par[1]*(x[0]-9.46) + par[2]*((x[0]-9.46)*(x[0]-9.46)) + par[3]*((x[0]-9.46)*(x[0]-9.46)*(x[0]-9.46));
}
...
Double_t PolOrder3(Double_t *x, Double_t*par){
    return par[0]+ par[1]*(x[0]-par[3]) + par[2]*(x[0]*x[0]-par[3]);
}
...
Double_t v2vsMassThreeIndCB2VWGCheb(Double_t*x, Double_t *par){ // bkg 7 + sig 21 + flow bkg 5 + flow sig 1 = 34
    return (backFcnVWGaus(x, par)*v2BackgroundChebY(x, &par[28]) + par[33]*ThreeIndCrystalBallExtended(x, &par[7]))/(backFcnVWGaus(x, par) + ThreeIndCrystalBallExtended(x, &par[7]));
}
...
Double_t v2vsMassTwoCB2VWGCheb(Double_t*x, Double_t *par){ // bkg 4 + sig 8 + flow bkg 5 + flow sig 1 = 18
    return (backFcnVWGaus(x, par)*v2BackgroundChebY(x, &par[12]) + par[17]*TwoCrystalBallExtended(x, &par[4]))/(backFcnVWGaus(x, par) + TwoCrystalBallExtended(x, &par[4]));
}
...
//_______________________________________________________________________________
Double_t v2eventbyevent(Double_t*x, Double_t *par){
  return (2.0 * par[0] * x[0] * TMath::Power(1 - x[0] * x[0], (par[0] - 1.0) ) );

}

//____________________________________
Double_t dedx(Double_t *x, Double_t *par){
  //input mass
  //par[0]: mass
  //resolution 5%
  //keep all parameters constant except of mass itself
  Double_t energ = TMath::Sqrt(x[0]*x[0] + par[0]*par[0]);
  Double_t beta = x[0]/energ;
  Double_t betagamma = x[0]/par[0];
  //need to account for probability as function of momentum
  //arbitrary normalisation
  
  return 1.55081/50./*P1*/ *
    (26.3394/*P2*/ - TMath::Power(beta,2.095/*P4*/) - TMath::Log( 5.04114e-11/*P3*/ + 1.0 / (TMath::Power(betagamma, 2.385/*P5*/))
						)
     )/ TMath::Power(beta, 2.095/*P4*/);
}
//________________________________________________                                                                                                                                                                                                                  
Double_t spectra(Double_t *x, Double_t *par)
{
  Double_t energ = TMath::Sqrt(x[0]*x[0] + par[0]*par[0]);
  Double_t value = x[0] * x[0] * TMath::Exp( - 1.0 * energ / par[1] );
  return value;
}

//____________________________________                                                                                                           
Double_t dedx2D(Double_t *x, Double_t *par){
  //input mass
  //par[0]: mass
  //resolution 5%
  //input arguments: 1) momentum, 2) dedx
  //keep all parameters constant except of mass itself
  //Double_t energ = TMath::Sqrt(x[0]*x[0] + par[0]*par[0]);
  //Double_t beta = x[0]/energ;
  //Double_t betagamma = x[0]/par[0];
  //need to account for probability as function of momentum
  //arbitrary normalisation
  Double_t dedx_mean = dedx(x, par);
  Double_t norm_sigma = (dedx_mean - x[1]) / (dedx_mean  * 0.05);
  return TMath::Gaus(0,norm_sigma);
