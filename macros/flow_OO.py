...
def main():
    ...
    # Set marker size to 1.0
    SetGraStat(graStatV2VsPtOO, 20, ROOT.kRed+1, 1.0)
    SetGraSyst(graSystV2VsPtOO, 20, ROOT.kRed+1) # SetGraSyst size is handled in plot_library.py to be 0
    ...
    # Set marker size to 1.0
    SetGraStat(graStatV2VsPtPbPb, 20, ROOT.kBlack, 1.0)
    SetGraSyst(graSystV2VsPtPbPb, 20, ROOT.kBlack) # Removed size 0.7
    ...
    # Set marker size to 1.0
    SetGraStat(graStatV2VsPtpPb, 20, ROOT.kAzure+4, 1.0)
    SetGraSyst(graSystV2VsPtpPb, 20, ROOT.kAzure+4) # Removed size 0.7
    ...
    # Set marker size to 1.0
    SetGraStat(graStatV2VsPtPbp, 20, ROOT.kGreen+1, 1.0)
    SetGraSyst(graSystV2VsPtPbp, 20, ROOT.kGreen+1) # Removed size 0.7
    ...
    #legendV2VsPt.AddEntry(graStatV2VsPtPbPb, "#sqrt{#it{s}} = 5.02 TeV, Pb-Pb, 10#minus30%", "FP")
    legendV2VsPt.AddEntry(graStatV2VsPtpPb, "#sqrt{#it{s}} = 8.16 TeV, pPb, 2.03 < #it{y} < 3.53", "FP")
    legendV2VsPt.AddEntry(graStatV2VsPtPbp, "#sqrt{#it{s}} = 8.16 TeV, pPb, -4.46 < #it{y} < -2.96", "FP")
