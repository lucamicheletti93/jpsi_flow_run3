...
double minRapBins[] = {2.5,2.75,3.0,3.25,3.50,3.75};
double maxRapBins[] = {2.75,3.0,3.25,3.50,3.75,4.0};
double RapBins[] = {2.5,2.75,3.0,3.25,3.50,3.75,4.0};
//double minRapBins[] = {3};
//double maxRapBins[] = {4};

//const int nRapBins = 7;
//double minRapBins[] = {2.5,2.5,2.75,3.0,3.25,3.50,3.75};
//double maxRapBins[] = {4.0,2.75,3.0,3.25,3.50,3.75,4.0};
...
string dirPath = "/Users/dhananjaya/Desktop/ALICE_PbPb_Analysis/jpsi_flow_run3/y_dependence";

void v2_fitter_RapDep() {
  ...
  TH1D *histStatJpsiV2 = new TH1D("histStatJpsiV2", "", 6, RapBins);
  histStatJpsiV2 -> GetXaxis() -> SetTitle("y");
  ...
  TH1D *histSystJpsiV2 = new TH1D("histSystJpsiV2", "", 6, RapBins);
  ...
  for (int iRap = 0;iRap < nRapBins;iRap++) {
    ...
    double minRapBin = minRapBins[iRap];
    double maxRapBin = maxRapBins[iRap];
    nTrials = 0;

    TFile *fIn = new TFile("RapDepHistograms_matchedMchMid_centr_10_50.root");
    TFile *fOut = new TFile(Form("systematics/fitResults_Rap_%.2f_%.2f.root", minRapBin, maxRapBin), "RECREATE");
    fIn->ls();
    for (int iSigFunc = 0;iSigFunc < nSigFuncs;iSigFunc++) { // loop on mass signal funcs
      for (int iBkgFunc = 0;iBkgFunc < nBkgFuncs;iBkgFunc++) { // loop on mass background funcs
        for (int iTailSet = 0;iTailSet < nTailSets;iTailSet++) { // loop on tail parameters
         ...
         TFile *fInMassFit = new TFile(Form("y_dependence/Rap_%.2f_%.2f/multi_trial_%s_%s_%s_tails.root", minRapBin, maxRapBin, sigFunc.c_str(), bkgFunc.c_str(), tailSet.c_str()));
         fInMassFit->ls();
          for (int iFitRange = 0;iFitRange < nFitRanges;iFitRange++) {
            ...
            TH1D *histMassFitPars = (TH1D*) fInMassFit -> Get(Form("fit_results_%s_%s__%.1f_%.1f_histMassSEPM_0.00_30.00_%.2f_%.2f__10.00_50.00", sigFunc.c_str(), bkgFunc.c_str(), minFitRange, maxFitRange, minRapBin, maxRapBin));
            TH1D *histMass = (TH1D*) fIn -> Get(Form("histMassSEPM_0.00_30.00_%.2f_%.2f__10.00_50.00", minRapBin, maxRapBin));
            TProfile *profV2 = (TProfile*) fIn -> Get(Form("histV2SEPM_0.00_30.00__%.2f_%.2f__10.00_50.00", minRapBin, maxRapBin));
            ...
            for (int iBkgV2Func = 0;iBkgV2Func < nBkgV2Funcs;iBkgV2Func++) { // loop on v2 background types
              ...
              if (iSigFunc == 0 && iBkgFunc == 0 && iTailSet == 0 && iFitRange == 0 && iBkgV2Func == 0) {
                results = DoFlowFit(histMassFitPars, histMass, profV2, minRapBin, maxRapBin, minFitRange, maxFitRange, kTRUE, Form("%s_%s_%s_%s_v2bkg", sigFunc.c_str(), bkgFunc.c_str(), tailSet.c_str(), bkgV2Func.c_str()), fOut);
              } else {
                results = DoFlowFit(histMassFitPars, histMass, profV2, minRapBin, maxRapBin, minFitRange, maxFitRange, kFALSE, Form("%s_%s_%s_%s_v2bkg", sigFunc.c_str(), bkgFunc.c_str(), tailSet.c_str(), bkgV2Func.c_str()), fOut);
              }
    ...
    histStatJpsiV2 -> SetBinContent(iRap+1, meanJpsiV2);
    histStatJpsiV2 -> SetBinError(iRap+1, meanStatErrJpsiV2);
    histSystJpsiV2 -> SetBinContent(iRap+1, meanJpsiV2);
    histSystJpsiV2 -> SetBinError(iRap+1, meanSystErrJpsiV2);
    ...
    //TText *text1 = display1 -> AddText(Form(" v_{2}^{J/#psi} [%1.0f < #it{p}_{T} < %1.0f GeV/#it{c}] = %5.4f #pm %5.4f (%4.3f %%) #pm %5.4f (%4.3f %%)", minRapBin, maxRapBin, meanJpsiV2, meanStatErrJpsiV2, meanStatErrJpsiV2Perc, meanSystErrJpsiV2, meanSystErrJpsiV2Perc));
    TText *text1 = display1 -> AddText(Form(" v_{2}^{J/#psi} [%0.2f < #it{y} < %0.2f GeV/#it{c}] = %5.4f #pm %5.4f #pm %5.4f", minRapBin, maxRapBin, meanJpsiV2, meanStatErrJpsiV2, meanSystErrJpsiV2));
    display1 -> Draw("same");

    canvasSyst -> Write();
    histChi2Ndf -> Write();
    canvasSyst -> SaveAs(Form("%s/systematic_plot_Pt_%0.2f_%0.2f.pdf", dirPath.c_str(), minRapBin, maxRapBin));
  ...
  for (int iy = 0;iy < nRapBins;iy++) {
    Printf("%3.2f %3.2f %6.5f %6.5f %6.5f ", minRapBins[iy], maxRapBins[iy], histStatJpsiV2 -> GetBinContent(iy+1), histStatJpsiV2 -> GetBinError(iy+1), histSystJpsiV2 -> GetBinError(iy+1));
  }
}
////////////////////////////////////////////////////////////
double* DoFlowFit(TH1D *histMassFitPars, TH1D *histMass, TProfile *profV2, double minRapBin, double maxRapBin, double minFitRange, double maxFitRange, bool debug, string fitFuncs, TFile *fOut) {
  ...
  if (debug) {
    canvasFit -> SaveAs(Form("%s/v2_fit_%2.1f_%2.1f_%s_Pt_%0.2f_%0.2f.pdf", dirPath.c_str(), minFitRange, maxFitRange, fitFuncs.c_str(), minRapBin, maxRapBin));
  }
  return results;
}
...
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
std::vector<TH1*> Uncertainities(std::vector<TH1*> histlist, vector<double> parameter, vector<double> parameter_er, double value[nRapBins][3]) {
    std::vector<TH1*> histograms;
    int ncombination = parameter.size() / nRapBins;
    int nbin = 0;
